...
import { expectCalls, L2_NOW_AMER_IDS, L2_NOW_IDS, l2Calls } from "../helpers/alertCards";
...
describe("loadClosureComposition (spec §9 4.6; D11, D12)", () => {
  ...
  it.each([
    ...
    const deps = fakeDeps();
    const out = await loadClosureComposition(sel({ window: key }), null, deps);
  ...
  it("alertType at 7 / 30 / now", async () => {
    ...
    const seven = await loadClosureComposition(sel({ window: 7 }), "alertType", deps);
    ...
    const thirty = await loadClosureComposition(sel({ window: 30 }), "alertType", deps);
    ...
    const now = await loadClosureComposition(sel({ window: "now" }), "alertType", deps);
  ...
  it("routingPersona uses the closed event's persona (A55 → Logistics)", async () => {
    ...
    const seven = await loadClosureComposition(sel({ window: 7 }), "routingPersona", deps);
    expect(byGroup(seven.raw.closedTotalByGroup)).toEqual([
    ...
    const thirty = await loadClosureComposition(sel({ window: 30 }), "routingPersona", deps);
    expect(byGroup(thirty.raw.closedTotalByGroup)).toEqual([
  ...
  it("priority uses priorityAtEvent of the closed event (A55 → Medium)", async () => {
    ...
    const seven = await loadClosureComposition(sel({ window: 7 }), "priority", deps);
    expect(byGroup(seven.raw.closedTotalByGroup)).toEqual([
    ...
    const now = await loadClosureComposition(sel({ window: "now" }), "priority", deps);
    expect(byGroup(now.raw.closedTotalByGroup)).toEqual([
  ...
  it("AMER filter: closed alerts on AMER items; facts = the 18 AMER L2('now') alerts", async () => {
    ...
    const seven = await loadClosureComposition(sel({ window: 7, filters: AMER }), null, deps);
    ...
    const thirty = await loadClosureComposition(sel({ window: 30, filters: AMER }), null, deps);
  ...
  it("truncated when the grouped call returns MAX_GROUPS rows; row-cap from L2", async () => {
    ...
    const out = await loadClosureComposition(sel({ window: 7 }), "routingPersona", deps3);
    expect(out).toMatchObject({ status: "ok", caveats: [] });
    expect(deriveClosureComposition(out.raw, sel({ window: 7 }), deps3.config).caveats).toContain("truncated");
    clearMetricsCache();
    // MAX_GROUPS 4 → 3 groups < 4 → not truncated.
    const deps4 = fakeDeps({ config: { MAX_GROUPS: 4 } });
    const four = await loadClosureComposition(sel({ window: 7 }), "routingPersona", deps4);
    expect(deriveClosureComposition(four.raw, sel({ window: 7 }), deps4.config).caveats).not.toContain("truncated");
    clearMetricsCache();
    // ROW_CAP 5: L1("now") 66 rows → capped.
    const capped = await loadClosureComposition(sel({ window: 7 }), null, fakeDeps({ config: { ROW_CAP: 5, PAGE_SIZE: 5 } }));
