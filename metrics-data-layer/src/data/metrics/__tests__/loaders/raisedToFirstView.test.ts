...
describe("loadRaisedToFirstView (spec §9 4.3; D11, D12)", () => {
  ...
  it("7 days: L2 over the SELECTED window, no not-worked, no items", async () => {
    const deps = fakeDeps();
    const out = await loadRaisedToFirstView(sel({ window: 7 }), null, deps);
  ...
  it("30 days: 34 touched alerts", async () => {
    const deps = fakeDeps();
    const out = await loadRaisedToFirstView(sel({ window: 30 }), null, deps);
  ...
  it("'now': all 46 touched alerts", async () => {
    const deps = fakeDeps();
    const out = await loadRaisedToFirstView(sel({ window: "now" }), null, deps);
  ...
  it.each(ALERT_DIMS)("alert dim %s: no itemsById", async (dim) => {
    const deps = fakeDeps();
    const out = await loadRaisedToFirstView(sel({ window: 7 }), dim, deps);
  ...
  it("item dim at 30 days and 'now'", async () => {
    const deps = fakeDeps();
    const thirty = await loadRaisedToFirstView(sel({ window: 30 }), "productLine", deps);
    ...
    const now = await loadRaisedToFirstView(sel({ window: "now" }), "productLine", deps);
  ...
  it("row cap → partial + row-cap", async () => {
    ...
    const out = await loadRaisedToFirstView(sel({ window: 7 }), null, fakeDeps({ config: { ROW_CAP: 5, PAGE_SIZE: 5 } }));
