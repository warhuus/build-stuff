...
import { sel } from "./alertCardTestUtils";
...
describe("section-4 loaders feed their derives (7 days unless noted)", () => {
  ...
  it("4.2: worked 6 (A42 A44 A46 A49 A50 A55 closed in 7 d), not worked 2 (A35 A36; A40 has no opened event)", async () => {
    const out = await loadRaisedToClosed(sel(7), null, fakeDeps());
    const d = deriveRaisedToClosed(out.raw, sel(7), FIXTURE_CONFIG);
  ...
  it("4.3: 12 alerts first viewed in 7 d (A09 A13 A15 A19 A25 A44 A46 A49 A53 A58 A62 A65)", async () => {
    const out = await loadRaisedToFirstView(sel(7), null, fakeDeps());
    expect(deriveRaisedToFirstView(out.raw, sel(7), FIXTURE_CONFIG).data.total.series[0].n).toBe(12);
  });

  it("4.4: 6 viewed alerts closed in 7 d (A44 view = close stamp counts as before)", async () => {
    const out = await loadFirstViewToClosure(sel(7), null, fakeDeps());
    expect(deriveFirstViewToClosure(out.raw, sel(7), FIXTURE_CONFIG).data.total.series[0].n).toBe(6);
  });

  it("4.5 (30): 48 open alerts, 3 of unknown age (A32–A34 raised before the pipeline start)", async () => {
    const out = await loadAgeingBacklog(sel(30), null, fakeDeps());
    const d = deriveAgeingBacklog(out.raw, sel(30), FIXTURE_CONFIG);
  ...
  it("4.6: closedTotal 9 = writeBack A50 + action A46 A49 + viewOnly A42 A44 A55 + noHuman 3", async () => {
    const out = await loadClosureComposition(sel(7), null, fakeDeps());
    const d = deriveClosureComposition(out.raw, sel(7), FIXTURE_CONFIG);
