...
import { expectCalls, itemsCall, L2_NOW_AMER_IDS, L2_NOW_IDS, l2Calls } from "../helpers/alertCards";
...
describe("loadFirstViewToClosure (spec §9 4.4; D11, D12)", () => {
  ...
  it("facts carry first view and closure: A44 view at the close stamp, A43 closed 20 days ago", async () => {
    const out = await loadFirstViewToClosure(sel({ window: 30 }), null, fakeDeps());
  ...
  it.each(ALERT_DIMS)("alert dim %s: no itemsById", async (dim) => {
    const deps = fakeDeps();
    const out = await loadFirstViewToClosure(sel({ window: 7 }), dim, deps);
  ...
  it.each(ITEM_DIMS)("item dim %s at 30 days: items of the 4.4 population only (L5)", async (dim) => {
    const deps = fakeDeps();
    const out = await loadFirstViewToClosure(sel({ window: 30 }), dim, deps);
  ...
  it("AMER at 7 days with businessLine", async () => {
    const deps = fakeDeps();
    const out = await loadFirstViewToClosure(sel({ window: 7, filters: AMER }), "businessLine", deps);
  ...
  it("row cap with an item dim → partial + row-cap", async () => {
    ...
    const out = await loadFirstViewToClosure(sel({ window: 7 }), "region", fakeDeps({ config: { ROW_CAP: 30, PAGE_SIZE: 10 } }));
