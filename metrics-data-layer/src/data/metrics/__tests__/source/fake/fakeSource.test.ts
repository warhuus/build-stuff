...
const ctx = (over: Partial<MetricsConfig> = {}) => ({
  signal: new AbortController().signal,
  config: { ...FIXTURE_CONFIG, ...over },
});
...
describe("fake source: item aggregates", () => {
  it("drops null group values and sums values ignoring nulls", async () => {
    ...
    expect(await src.countItemsBy(allItems, "businessLine", ctx())).toEqual([
  ...
  it("truncates to MAX_GROUPS keeping the largest, ties by name", async () => {
    ...
    const groups = await src.countItemsBy(allItems, "plant", ctx({ MAX_GROUPS: 2 }));
    expect(groups.map((g) => [g.group, g.count])).toEqual([
      ["P100", 13],
      ["P200", 13],
    ]);
    const regions = await src.countItemsBy(allItems, "region", ctx({ MAX_GROUPS: 1 }));
  ...
  it("returns count 0 and value 0 for an empty set", async () => {
    ...
    expect(await createFakeSource().countItems(empty, ctx())).toEqual({ count: 0, valueUsd: 0 });
  });
  ...
  it("applies one $in per non-empty filter dimension and ignores empty ones", async () => {
    ...
    expect((await createFakeSource().countItems(withItemFilters(allItems, f), ctx())).count).toBe(14);
  });
});
...
describe("fake source: events and open alerts", () => {
  it("under now has no lower bound (pre-pipeline human events count)", async () => {
    const src = createFakeSource();
    const viewed = (w: Window) => src.countEvents(eventsWhere(allEvents, ["viewed"], w), "alert", ctx());
  ...
  it("includes the window start instant (inclusive bounds)", async () => {
    ...
    const ids = (await createFakeSource().fetchEvents(opened7, ctx())).rows.map((r) => r.riskAlertId);
  ...
  it("counts distinct actors and groups by the mapped AlertHistory field", async () => {
    ...
    expect(await src.countEvents(actions7, "actor", ctx())).toBe(5);
    const byType = await src.countEventsBy(actions7, "alert", "actionType", ctx());
  ...
  it("holds only open alerts in AlertOrderFulfillment and maps escalated to labels (nulls dropped)", async () => {
    const src = createFakeSource();
    expect(await src.countOpenAlerts(allOpenAlerts, ctx())).toBe(48);
    expect(await src.countOpenAlertsBy(allOpenAlerts, "escalated", ctx())).toEqual([
  ...
  it("resolves the alert pivot only for alerts open now", async () => {
    ...
    expect(await src.countEvents(closedEvents, "alert", ctx())).toBe(25);
    const rows = (await src.fetchOpenAlerts(openAlertsOfEvents(closedEvents), ctx())).rows;
...
describe("fake source: risk, app usage, verdicts", () => {
  it("evaluates the bucket where-clauses (spec §9 3.1)", async () => {
    ...
    const counts = await Promise.all(
      (["unscored", "b15_30", "b31_50", "b51_70", "b71_90", "b91_100", "delayed"] as const).map((b) =>
        src.countRisk(riskBucket(riskAll(NONE), b), ctx()),
      ),
    );
  ...
  it("groups $ranges by range start, drops nulls, omits empty ranges", async () => {
    ...
    expect(await src.countRiskByScoreRange(riskNotDelayed(riskAll(NONE)), ranges, ctx())).toEqual([
    ...
    expect(await src.countRiskByScoreRange(riskBucket(riskAll(NONE), "b71_90"), ranges, ctx())).toEqual([
      { startValue: 71, count: 3 },
    ]);
  });
  ...
  it("counts app users of ALERT_APP_ID only, groups by queue filter without nulls", async () => {
    const src = createFakeSource();
    expect(await src.countAppUsers(W7, ctx())).toBe(3);
    expect(await src.countAppUsers(NOW, ctx())).toBe(6);
    expect(await src.countAppUsers(NOW, ctx({ ALERT_APP_ID: "other-app" }))).toBe(1);
    expect(await src.countAppUsersBy(W7, "queueFilter", ctx())).toEqual([
      { group: "All", count: 1 },
  ...
  it("gates verdict totals and compares the chosen verdict date on calendar dates", async () => {
    ...
    expect(await src.countVerdictsBy({ mode: "otif", window: W7 }, ctx())).toEqual([
      { group: "OTIF", count: 5 },
      { group: "Not OTIF", count: 1 },
    ]);
    const target = ctx({ VERDICT_DATE_PROPERTY: "otifFirstInitialDeliveryDateTarget" });
    expect(await src.countVerdictsBy({ mode: "otif", window: W7 }, target)).toEqual([
      { group: "OTIF", count: 4 },
      { group: "Not OTIF", count: 2 },
    ]);
    expect(await src.countVerdictsBy({ mode: "crit", window: W7 }, ctx())).toEqual([
      { group: "CRIT", count: 6 },
      { group: "Not CRIT", count: 1 },
    ]);
    expect(await src.countVerdictsBy({ mode: "otif", window: NOW }, ctx())).toEqual([
      { group: "OTIF", count: 7 },
  ...
  it("returns no verdict totals while the date property is a placeholder", async () => {
    const placeholder = ctx({ VERDICT_DATE_PROPERTY: PLACEHOLDER });
  ...
  it("records every call with its arguments", async () => {
    const src = createFakeSource();
    await src.countItemsBy(allItems, "plant", ctx());
    await src.fetchItemsByIds([], ctx());
