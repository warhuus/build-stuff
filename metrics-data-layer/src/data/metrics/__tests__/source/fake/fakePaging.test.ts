...
const ctxWith = (over: Partial<MetricsConfig>, onProgress?: (p: Progress) => void, signal = new AbortController().signal) => ({
  signal,
  onProgress,
...
describe("fake source: paging (spec §9.0 fetchAllPages)", () => {
  it("pages by PAGE_SIZE and reports cumulative progress after each page", async () => {
    ...
    const res = await src.fetchItems(allItems, ctxWith({ PAGE_SIZE: 15 }, (p) => progress.push(p.loaded)));
  ...
  it("stops at ROW_CAP with capped true and rows truncated to the cap", async () => {
    const progress: number[] = [];
    const res = await createFakeSource().fetchEvents(allEvents, ctxWith({ PAGE_SIZE: 10, ROW_CAP: 25 }, (p) => progress.push(p.loaded)));
  ...
  it("is not capped below the cap", async () => {
    const res = await createFakeSource().fetchEvents(allEvents, ctxWith({ PAGE_SIZE: 100, ROW_CAP: TOTAL_EVENTS + 1 }));
    expect(res).toMatchObject({ capped: false });
  ...
  it("checks abort between pages and rejects with an AbortError", async () => {
    ...
    const src = createFakeSource();
    const run = src.fetchEvents(allEvents, ctxWith({ PAGE_SIZE: 10 }, () => controller.abort(), controller.signal));
  ...
  it("rejects an already aborted aggregate", async () => {
    const controller = new AbortController();
    controller.abort();
    await expect(createFakeSource().countItems(allItems, ctxWith({}, undefined, controller.signal))).rejects.toMatchObject({
      name: "AbortError",
    });
  });
});
...
describe("fake source: id lookups (spec §9.0 itemsById, §9 4.1)", () => {
  it("returns nothing and makes no batch for zero ids (decision D14)", async () => {
    const src = createFakeSource();
    expect(await src.fetchItemsByIds([], ctxWith({}))).toEqual({ rows: [], capped: false });
    expect(await src.fetchVerdictsByIds([], ctxWith({}))).toEqual({ rows: [], capped: false });
  ...
  it("chunks by ID_BATCH with at most INNER_CONCURRENCY in flight; missing ids are absent", async () => {
    ...
    const res = await src.fetchItemsByIds(ids, ctxWith({ ID_BATCH: 3, INNER_CONCURRENCY: 2 }));
  ...
  it("never exceeds the default INNER_CONCURRENCY", async () => {
    const src = createFakeSource();
    await src.fetchItemsByIds(ITEM_IDS, ctxWith({ ID_BATCH: 1 }));
    expect(src.stats.maxInFlight).toBe(FIXTURE_CONFIG.INNER_CONCURRENCY);
  });

  it("caps id lookups at ROW_CAP across chunks", async () => {
    const res = await createFakeSource().fetchItemsByIds(ITEM_IDS, ctxWith({ ID_BATCH: 10, ROW_CAP: 25 }));
    expect(res.capped).toBe(true);
    expect(res.rows).toHaveLength(25);
  });
  ...
  it("checks abort between chunks", async () => {
    const controller = new AbortController();
    const src = createFakeSource();
    const run = src.fetchItemsByIds(ITEM_IDS, ctxWith({ ID_BATCH: 5, INNER_CONCURRENCY: 1 }, () => controller.abort(), controller.signal));
    await expect(run).rejects.toMatchObject({ name: "AbortError" });
  ...
  it("looks verdicts up per VERDICT_ID_LOOKUP and maps the chosen verdict date", async () => {
    ...
    const rows = (await byIn.fetchVerdictsByIds(ids, ctxWith({ VERDICT_ID_LOOKUP: "in" }))).rows;
    ...
    const target = ctxWith({ VERDICT_ID_LOOKUP: "eq", VERDICT_DATE_PROPERTY: "otifFirstInitialDeliveryDateTarget" });
