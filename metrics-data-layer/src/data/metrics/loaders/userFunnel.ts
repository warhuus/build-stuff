...
import { QUERIED_USER_STAGES, eventGroupFieldOf, isQueriedUserStage, stagesForDim } from "../breakdowns";
import { escalatedFlagLabel } from "../compute/dimValues";
...
import type { BreakdownDimension, GroupCount, StageId, UserFunnelRaw, UserStageId, Window } from "../types";
...
const isEventStage = (id: StageId): id is EventStageId => id !== "1.1" && isQueriedUserStage(id);
/** 1.2–1.4 in config order (`FUNNEL_STAGES.user`). */
const EVENT_STAGES: readonly EventStageId[] = FUNNEL_STAGES.user.filter(isEventStage);
 ...
 * `countEvents` per value; groups labelled by `escalatedFlagLabel` (both always present in the raw; derive drops a
 * zero-count label, COR-03).
 */
async function escalatedGroups(stage: EventStageId, w: Window, source: MetricsSource, ctx: SourceCtx): Promise<GroupCount[]> {
  ...
  return [
    { group: escalatedFlagLabel(true, ctx.config), count: yes },
    { group: escalatedFlagLabel(false, ctx.config), count: no },
  ];
}
...
async function stageGroups(
  ...
  if (stage === "1.1") {
    if (dim !== "queueFilter") throw new RangeError(`stage 1.1 has no ${dim} breakdown`);
    return { stage, rows: await source.countAppUsersBy(w, dim, ctx) };
  }
  if (dim === "escalated") return { stage, rows: await escalatedGroups(stage, w, source, ctx) };
  const field = eventGroupFieldOf(dim);
  if (field === null) throw new RangeError(`${dim} is not a userFunnel breakdown`);
  return { stage, rows: await source.countEventsBy(stageEvents(stage, w), "actor", field, ctx) };
}
...
function allStageGroups(dim: BreakdownDimension | null, w: Window, source: MetricsSource, ctx: SourceCtx): Promise<StageGroups[]> {
  ...
  const stages = QUERIED_USER_STAGES.filter((s) => applicable.includes(s));
 ...
 * @returns raw distinct users per stage and groups per applicable stage (null without a dim); status always
 * "ok" with no caveats (no row fetches; `truncated` is derive's, MOD-02).
