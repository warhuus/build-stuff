...
import { QUERIED_USER_STAGES, firstApplicableStage, stagesForDim } from "../breakdowns";
import { hasFilters } from "../selection";
import { countOfGroup, nonEmptyGroups } from "./breakdown";
 ...
 * (spec §9 1.0) and the stages the dim does not apply to `not-applicable`. Zero-count groups are dropped
 * before top-N (escalated always has both labels in the raw; COR-03).
 */
function userBreakdown(
  ...
  const groupStages = (group: string): FunnelStageRaw[] => [
    ...
    ...stagesFor(QUERIED_USER_STAGES, applicable, (id) =>
  ...
  const spec: FunnelBreakdownSpec = {
    dimension,
    additive: false,
    ranking: nonEmptyGroups(stageLookup(byStage, first) ?? []),
    groupStages,
...
/**
 * The grouped server results of the raw, for the `MAX_GROUPS` truncation check (spec §9.0 Truncation): every
 * stage's list, except escalated, whose two rows come from two ungrouped counts (SPF-06, MOD-02).
 */
function groupedCallLists(raw: UserFunnelRaw): readonly (readonly GroupCount[] | undefined)[] {
  return raw.dimension === "escalated" ? [] : QUERIED_USER_STAGES.map((id) => raw.groups?.[id]);
}

/**
 * userFunnel derive (spec §9 1.0–1.4): total = 1.0 `no-source` + 1.1–1.4 distinct users (valueUsd null); with a dim, the
 ...
 * (escalated) and `truncated` (top-N cut, or a grouped call returned `MAX_GROUPS` rows; `groupedCallLists`).
 */
export const deriveUserFunnel: Derive<UserFunnelRaw, FunnelSeries> = (raw, selection, config = METRICS_CONFIG) => {
  ...
  const stages = [
    STAGE_1_0,
    ...QUERIED_USER_STAGES.map((id) => okStage(id, { count: raw.users[id], valueUsd: null }, userStageCaveats(id, raw.window.key))),
  ];
