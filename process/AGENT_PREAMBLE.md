...
- Read /home/user/build-stuff/process/PHASE1_DECISIONS.md (FROZEN interfaces + lead decisions D1–D24; they override DESIGN.md) and /home/user/build-stuff/process/DESIGN.md before coding.
- Frozen files you must NOT edit: src/config/metrics.ts, metricsCodes.ts, metricsText.ts, src/data/metrics/types.ts, outputTypes.ts, rowTypes.ts, rawTypes.ts, query/specs.ts, source/MetricsSource.ts. Need a change? Write it under "Interface change requests" in your findings file and continue with everything else.
- Naming conventions every agent relies on:
  - Loaders: `src/data/metrics/loaders/<cardId>.ts` exports `export const load<CardId>: Loader<CardRaw["<cardId>"]>` (e.g. `loadUserFunnel`, `loadItemFunnel`, `loadRiskDistribution`, `loadOtifOutcome`, `loadRaisedToClosed`, `loadRaisedToFirstView`, `loadFirstViewToClosure`, `loadAgeingBacklog`, `loadClosureComposition`); `loaders/blocked.ts` exports `blockedResult(...)` helpers for stub/placeholder cards.
  - Derives: `src/data/metrics/compute/derive<CardFamily>.ts` exports `export const derive<CardId>: Derive<CardRaw["<cardId>"], CardOutput["<cardId>"]>` for the same nine card ids (durations may share one file `deriveDurations.ts` exporting three derives).
  - Fixtures: `src/data/metrics/source/fake/fixtures.ts` exports `FIXTURE_NOW: Date` and `FIXTURES` (the dataset), and `createFakeSource(data = FIXTURES): MetricsSource` lives in `fakeSource.ts`.
- Findings file: /home/user/build-stuff/process/phase2-<agent>.md (what you built, decisions/assumptions to record in QUESTIONS.md with spec refs, interface change requests, anything unfinished).
- Other agents are writing other folders at the same time. Only run tsc/eslint/vitest; if tsc errors appear in files you do not own, ignore them (report if they block you). Never delete or rewrite others' files.
